package Recursos;

import javax.swing.Icon;

public class Mensagens {
	public static final String ABRIR_CARTEIRA = "Abrir Carteira";
    public static final String ACAO = "Ação";
    public static final String ACAO_COMPRADA_COM_SUCESSO = "Ação Comprada com Sucesso";
    public static final String ACAO_NAO_COMPRADA = "Não foi possível concluir a transação";
    public static final String CADASTRAR_USUARIO = "Cadastrar Usuário";
    public static final String CANCELAR = "Cancelar";
    public static final String CARTEIRA_DE_ACOES = "Carteira de Ações";
    public static final String COMPRAR = "Comprar";
    public static final String CORRETAGEM = "Corretagem";
    public static final String CPF = "CPF";
    public static final String FACEBOOK = "Facebook";
    public static final String GERAL = "Geral";
    public static final String GOOGLE = "Google";
    public static final String IMPOSTO = "Imposto";
    public static final String INICIO = "Início";
    public static final String LOGIN = "Login";
    public static final String MSG_CARTEIRA_INEXISTENTE = "Não existe Login, por favor faça o Login para poder proceder";
    public static final String NOME = "Nome";
    public static final String OK = "Ok";
    public static final String POR_ACAO = "Por Ação";
    public static final String POR_PERIODO = "Por Período";
    public static final String QTD = "Quantidade";
    public static final String RELATORIOS = "Relatorios";
    public static final String REGISTRAR_TRANSACAO = "Registrar Transação";
    public static final String RESUMO_DA_CARTEIRA = "Resumo da Carteira";
    public static final String SALVAR = "Salvar";
    public static final String SEM_TRANSACOES = "Nenhuma transação registrada para este usuário!";
    public static final String TIPO = "Tipo:";
    public static final String TOTAL = "Total";
    public static final String TRANSACOES = "Transações";
    public static final String USUARIO_CADASTRADO = "Usuário Cadastrado com sucesso!";
    public static final String USUARIO_CARREGADO = "Usuário carregado com sucesso!";
    public static final String USUARIO_EXISTENTE = "Ja existe um usuário cadastrado com os dados informados!";
    public static final String USUARIO_NAO_ENCONTRADO = "Não existe usuário cadastrado com este CPF!";
    public static final String VALOR_UNITARIO = "Valor Unitário";	
    public static final String VENDER = "Vender";
    public static final String YOUTUBE = "Youtube";
}
